...
This is the volume that most linear algebra courses *are*, done with the
...
attention is written down in Volume VIII.
...
Two notebooks in this volume are more abstract than the rest, deliberately.
...
basis* depends on. The payoff is not aesthetic. Half of Volume III is the search
for a basis in which a matrix looks simple, and you cannot search for something
you have no word for.

One warning, which [§1.3](inverses-rank-cr.ipynb) makes concrete and Volume IV finally resolves. In exact
